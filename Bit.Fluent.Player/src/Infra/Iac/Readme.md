...
pulumi config set Bit.Fluent.Player.Iac:sql-server-td-db-admin-id
pulumi config set Bit.Fluent.Player.Iac:sql-server-td-db-admin-password --secret

# Provide SMTP server's host, port, user, pass and default email sender.
pulumi config set Bit.Fluent.Player.Iac:default-email-from
pulumi config set Bit.Fluent.Player.Iac:email-server-host
pulumi config set Bit.Fluent.Player.Iac:email-server-port
pulumi config set Bit.Fluent.Player.Iac:email-server-userName
pulumi config set Bit.Fluent.Player.Iac:email-server-password --secret

# Create and provide an identity certificate password
pulumi config set Bit.Fluent.Player.Iac:identity-certificate-password --secret
```
